...
const PATH_BASE = 'https://free.currencyconverterapi.com/api/v5';
...
class AmountInput extends React.Component {
  ...
  render() {
    return(
      <div>
          <input type="number" className="amount" value={this.props.value} onChange={(event) => this.props.convert(this.props.position, event)}></input>
        </div>
    );
  }
}
...
class CurrencySelector extends React.Component {
  ...
  render() {
    return(
      <div>
        <select className="currencySelector" value={this.props.selected_currency} onChange={(event) => this.props.recordCurrency(this.props.position, event)}>
          {this.props.currencies.map(item => 
            <option key={item.id} value={item.id}>{item.currencyName}</option>
          )}
        </select>
      </div>
    );
  ...
  render() {
    return(
      <div className="currency">
          <AmountInput value={this.props.value} convert={this.props.convert} position={this.props.position}/>
          <CurrencySelector currencies={this.props.currencies} recordCurrency={this.props.recordCurrency} selected_currency={this.props.selected_currency} position={this.props.position}/>
       </div>
    );
  }
}
...
class CurrencyGroupPair extends React.Component {
  ...
  getExchangeRate(source, destination) {
      ...
      let url = 'https://free.currencyconverterapi.com/api/v5/convert?q='+ source +'_' + destination + '&compact=y';

      // console.log(url);
      fetch(url)
      .then((response) => response.json())
      .then(data_response => this.updateExchangeRate(data_response, source, destination));
  }
  
  updateExchangeRate(message, source, destination) {
    console.log(message);
    this.setState({exchange_rate: message[source + "_" + destination]["val"]});    // update existing values based on new exchange rate
  ...
  importCurrencies(currencies) {
    ...
    this.setState({currencies: [this.state.currencies[0], this.state.currencies[1]]});
  }
    ...
    return(
      <div className="currency">
        {/* position parameter helps differentiate who is the top and who is the bottom group*/}
          <CurrencyGroup value={this.state.value[0]} convert={this.convert} currencies={this.state.all_currencies} recordCurrency={this.recordCurrency} position={0} selected_currency={this.state.currencies[0]}/>
          <CurrencyGroup value={this.state.value[1]} convert={this.convert} currencies={this.state.all_currencies} recordCurrency={this.recordCurrency}  position={1} selected_currency={this.state.currencies[1]} />
       </div>
    );
  }
}
